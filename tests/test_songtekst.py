"""Tests voor modules.songtekst (uitlijning van de officiële tekst)."""
...
def _segments() -> tuple[Segment, ...]:
    """Whisper hoorde: GEDENGEDENG (=kedeng kedeng), 'de trein' (=kedeng
    kedeng), en de gewone zin; de tweede kedeng-regel is gemist."""
...
def _lyrics(tmp_path: Path) -> Path:
    """Acht kedengs; de transcriptie kan er maar zes verklaren."""
...
def test_align_matches_gedengedeng_and_de_trein(tmp_path: Path) -> None:
    ...
    # Eerste twee kedengs -> GEDENGEDENG (2:1-koppeling).
    ...
    # 'kilometers spoor' als anker gewoon gekoppeld.
...
def test_extra_intervals_finds_new_and_estimates_gap(tmp_path: Path) -> None:
    ...
    known = [(1.0, 1.9)]  # de GEDENGEDENG-voorkomens zijn al bekend
    extras = extra_intervals(aligned, target_keys, known)
    labels = [label for _, _, label in extras]
    # 'de trein' is een nieuw gevonden kedeng-tijdvak...
    assert any(3.0 <= start <= 3.4 for start, _, _ in extras)
    # ...en de gemiste kedeng-regel wordt geschat tussen de ankers.
    assert any("geschat" in label for label in labels)
    # Bekende tijdvakken worden niet opnieuw gemeld.
...
def test_relabel_clusters_uses_official_text(tmp_path: Path) -> None:
    ...
    assert gedeng.label == "KEDENG"  # officiële tekst is leidend
