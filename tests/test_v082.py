"""Tests for v0.82.0 (B245, B247, B248, B249, B250, B241 follow-up)."""
...
def test_alignment_follows_gradual_drift() -> None:
    """B249: a gradual (falling) drift is not flattened out."""
...
def test_project_time_interpolates_between_regions() -> None:
    """B249: project_time interpolates linearly instead of stepping."""
    from modules.align import OffsetRegion, project_time
    regions = (OffsetRegion(0, 20, 0.0, 0.8),       # midpoint at 10 s
               OffsetRegion(20, 40, -2.0, 0.8))     # midpoint at 30 s
    # Halfway between the midpoints (20 s) the offset should be ~-1.0.
    proj = project_time(20.0, regions)
    assert abs((proj - 20.0) - (-1.0)) < 0.2


def test_weighted_anchor_prefers_reliable() -> None:
    """B250: a stronger anchor wins a conflict with a weaker one."""
    from modules.timing import Syllable, TimedLine, sanitize_timing

    def line(idx, text, start, quality):
        syl = Syllable(text=text, start=start, end=start + 0.5)
        return TimedLine(index=idx, text=text, crowd=False,
                         syllables=(syl,), quality=quality)

    # Line 2 (high) sits before line 1 (low): the reliable line 2 has to
    # set the sequence, not the weak line 1.
    ...
    assert starts == sorted(starts)                 # monotonic, no crash


def test_demucs_separate_cached_reuses(tmp_path, monkeypatch) -> None:
    """B248: separate_cached separates only once per source."""
    ...
    separation.separate_cached(src, cache, "original")   # reuse
    assert calls["n"] == 1


def test_font_store_key_roundtrip(tmp_path) -> None:
    """B245: a bundled font is kept portable (as a file name)."""
...
def test_language_on_the_go(tmp_path) -> None:
    ...
    # reloaded from the collection folder
    phonetics._EXTRA_LANGUAGES.clear()
